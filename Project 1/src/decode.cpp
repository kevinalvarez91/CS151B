...
std::shared_ptr<Instr> Core::decode(uint32_t instr_code) const {
  ...
  case InstType::I: {
    ...
    case Opcode::I:
      ...
      imm = static_cast<int32_t>(instr_code) >> 20; // TODO:
      break;
    case Opcode::L:
    case Opcode::JALR: {
      exe_flags.use_rd  = 1;
  ...
  case InstType::S: {
    ...
    exe_flags.use_imm = 1;
    exe_flags.alu_s2_imm = 1;
    uint32_t imm4_0 = (instr_code >> 7) & 0x1F;
    int32_t imm11_5 = static_cast<int32_t>(instr_code) >> 25;
    imm = (imm11_5 << 5) | imm4_0; // TODO:
  } break;
  ...
  case InstType::B: {
    ...
    exe_flags.use_imm = 1;
    exe_flags.alu_s2_imm = 1;
    uint32_t imm11 = ((instr_code >> 7) & 0x1) << 11;
    uint32_t imm4_1 = ((instr_code >> 8) & 0xF) << 1;
    uint32_t imm10_5 = ((instr_code >> 25) & 0x3F) << 5;
    uint32_t imm12 = ((instr_code >> 31) & 0x1) << 12;
    imm = imm12 | imm11 | imm10_5 | imm4_1;
    imm = (static_cast<int32_t>(imm) << 19) >> 19; // TODO:
  } break;
  ...
  case InstType::U: {
    ...
    exe_flags.use_imm = 1;
    exe_flags.alu_s2_imm = 1;
    imm = static_cast<int32_t>(instr_code) & 0xFFFFF000; // TODO:
  } break;
  ...
  case InstType::J: {
    ...
    exe_flags.use_imm = 1;
    exe_flags.alu_s2_imm = 1;
    // uint32_t imm20 = ((instr_code >> 31) & 0x1) << 20;
    // uint32_t imm19_12 = ((instr_code >> 12) & 0xFF) << 12;  
    // uint32_t imm11 = ((instr_code >> 20) & 0x1) << 11;
    // uint32_t imm10_1 = (instr_code >> 21) & 0x3FF;
    // imm = imm20 | imm19_12 | imm11 | imm10_1; 
    // imm = static_cast<int32_t>(imm << 11) >> 11;// TODO:
    int32_t imm20 = (instr_code >> 31) & 0x1;       // imm[20]: bit 31
    int32_t imm19_12 = (instr_code >> 12) & 0xFF;   // imm[19:12]: bits 12–19
    int32_t imm11 = (instr_code >> 20) & 0x1;       // imm[11]: bit 20
    int32_t imm10_1 = (instr_code >> 21) & 0x3FF;  // imm[10:1]: bits 21–30

    // Combine all parts into a 21-bit immediate (imm[20|19:12|11|10:1|0])
  ...
  case Opcode::R: {
    ...
    switch (func3) {
      ...
      case 2: alu_op = AluOp::LTI; break; // AluOp::SLT; -> VIA traces
      case 3: alu_op = AluOp::LTU; break; // AluOP::SLTU; -> VIA traces
