...
uint32_t Core::alu_unit(const Instr &instr, uint32_t rs1_data, uint32_t rs2_data, uint32_t PC) {
  ...
  case AluOp::SRA: {
    rd_data = static_cast<int32_t>(alu_s1) >> (alu_s2); // TODO:
    break;
  }
  case AluOp::LTI: {
    rd_data = static_cast<int32_t>(alu_s1) < static_cast<int32_t>(alu_s2) ? 1 : 0;// TODO:
    break;
  }
  case AluOp::LTU: {
...
uint32_t Core::branch_unit(const Instr &instr, uint32_t rs1_data, uint32_t rs2_data, uint32_t rd_data, uint32_t PC) {
  ...
  case BrOp::BLT: {//Signed comparison
    br_taken = (static_cast<int32_t>(rs1_data) < static_cast<int32_t>(rs2_data) ? 1 : 0); // TODO:
    break;
  }
  case BrOp::BGE: {//Signed comparison
    br_taken = (static_cast<int32_t>(rs1_data) >= static_cast<int32_t>(rs2_data));// TODO:
    break;
  }
  case BrOp::BLTU: {
    br_taken = (rs1_data < rs2_data); // TODO:
...
uint32_t Core::mem_access(const Instr &instr, uint32_t rd_data, uint32_t rs2_data) {
  ...
  if (exe_flags.is_load) {
    ...
    case 4: // RV32I: LBU
    case 5: // RV32I: LHU
      rd_data = read_data & ((1 << data_width) - 1); // TODO:
      break;
    default:
      std::abort();
    }
